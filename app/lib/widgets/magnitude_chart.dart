...
/// 측정 중 |a| 실시간 파형 스트립 차트.
/// 최신 값이 오른쪽 끝에 붙고 시간이 지나면 왼쪽으로 흘러간다.
/// 단일 시리즈 — 테마 primary 라인(2px) + 은은한 면 채움, Y축 자동 스케일.
class MagnitudeChart extends StatelessWidget {
  const MagnitudeChart({super.key, required this.values, this.height = 90});

  /// 200ms 버킷 peak 값 목록 (RecorderService.magnitudeHistory)
...
class _MagnitudePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // 최신 값을 오른쪽 끝에 고정, 왼쪽으로 스크롤
    final dx = plot.width / (_maxPoints - 1);
